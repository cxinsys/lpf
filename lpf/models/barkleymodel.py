...
class BarkleyModel(TwoComponentModel):
    """Barkley model (excitable media).
    f = (1/eps) * u * (1 - u) * (u - (v + b) / a)
    g = u - v
    params: [Du, Dv, eps, a, b]
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._name = "BarkleyModel"

    def reactions(self, t, u_c, v_c):
        batch_size = self.params.shape[0]

        eps = self.params[:, 2].reshape(batch_size, 1, 1)
        a   = self.params[:, 3].reshape(batch_size, 1, 1)
        b   = self.params[:, 4].reshape(batch_size, 1, 1)

        f = (1 / eps) * u_c * (1 - u_c) * (u_c - (v_c + b) / a)
        g = u_c - v_c
    ...
    def to_dict(self, index=0, params=None, initializer=None,
        ...
        n2v["a"] = float(params[index, 3])
        n2v["b"] = float(params[index, 4])
        return n2v
    ...
    def get_param_bounds(self):
        ...
        self.bounds_min[3] = -1; self.bounds_max[3] = 1    # a
        self.bounds_min[4] = -1; self.bounds_max[4] = 1    # b
