...
class LiawModel(ReactionDiffusionModel):
    ...
    def f(self, dx, u, v, u_c, v_c, Du, ru, k, su, mu):
        return Du * self.laplacian2d(u, dx) + (ru * ((u_c ** 2 * v_c) / (1 + k * u_c ** 2)) + su - mu * u_c)
        # return dt * (Du * laplacian2d(u, dx) + (ru * ((u_c*u_c * v_c) / (1 + k * u_c*u_c)) + su - mu * u_c))

    def g(self, dx, u, v, u_c, v_c, Dv, rv, k, sv):
        return Dv * self.laplacian2d(v, dx) + (-rv * ((u_c ** 2 * v_c) / (1 + k * u_c ** 2)) + sv)
    ...
    def pdefunc(self, t, y_linear):
        ...
        self._f = self.f(dx, u, v, u_c, v_c, Du, ru, k, su, mu)
        self._g = self.g(dx, u, v, u_c, v_c, Dv, rv, k, sv)
    ...
    def save_model(self,
        ...
        with open(fpath, "wt") as fout:
            ...
            # n2v["solver"] = self._solver.name if self._solver else None
