"""Shared fixtures for critical bug regression tests."""
...
@pytest.fixture
def grayscott_params():
    """Minimal Gray-Scott model parameters (batch_size=1, 4 params)."""
    return np.array([[2e-5, 5e-6, 0.04, 0.06]], dtype=np.float32)
...
@pytest.fixture
def grayscott_model(grayscott_params):
    """A fully initialized Gray-Scott model ready for solving."""
    from lpf.models import GrayScottModel
    from lpf.initializers import LiawInitializer

    init_pts = np.array([[[16, 16]]], dtype=np.uint32)
    init_states = np.array([[1.0, 0.0]], dtype=np.float32)
    initializer = LiawInitializer(init_states=init_states, init_pts=init_pts)

    model = GrayScottModel(
        initializer=initializer,
        params=grayscott_params,
        width=32,
        height=32,
        dx=0.1,
        device="cpu",
