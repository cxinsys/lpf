"""is_early_stopping() must not raise AttributeError.
...
class TestEarlyStopping:
    ...
    def test_early_stopping_before_pdefunc_raises(self, liaw_model):
        """Without pdefunc(), _f/_g don't exist → should raise AttributeError."""
        with pytest.raises(AttributeError):
            liaw_model.is_early_stopping(rtol=1e-6)
    ...
    def test_f_g_shape_matches_interior(self, liaw_model):
        ...
        batch_size = liaw_model.batch_size
        expected_shape = (batch_size, liaw_model._height - 2, liaw_model._width - 2)
